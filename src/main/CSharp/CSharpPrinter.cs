...
namespace CSharp
{
	public class CSharpPrinter : CSVisitor
	{
		...
		private List<CSUsing> PrintableUsingList(List<CSUsing> usings)
		{
			List<CSUsing> list = new List<CSUsing>();

			foreach (CSUsing u in usings)
			{
				list.Add(u);
			}

			list.Sort(delegate(CSUsing a, CSUsing b)
			{
				if (a.Namespace.StartsWith("System") && b.Namespace.StartsWith("System")) return a.Namespace.CompareTo(b.Namespace);
				else if (a.Namespace.StartsWith("System")) return -1;
				else if (b.Namespace.StartsWith("System")) return 1;
		...
		static Regex META_VARIABLE_PATTERN = new Regex("\\$(\\w+)");


		public override void Visit(CSMacroExpression node)
		{
			node.Macro.Accept(this);
		}

		public override void Visit(CSMacroTypeReference node)
	{
			node.Macro.Accept(this);
		}
